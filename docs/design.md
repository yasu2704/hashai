...
初期バージョンはsingle-turnのみとする。同じ入力と環境から予測しやすい結果を得られ、誤った過去出力が将来の変換へ影響しないためである。

将来multi-turnを導入する場合は、次を必須とする。
...
生成後に専用UIを設ける場合は次を候補とする。

- Execute: コマンドを入力バッファへ確定する。
- Describe: コマンドを説明する。
- Revise: 修正指示を与えて再生成する。
- Copy: クリップボードへコピーする。
- Abort: 元の入力へ戻す。

Coreから直接実行する機能は初期バージョンに含めない。
